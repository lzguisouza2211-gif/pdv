...
export function Cardapio() {
  ...
  // Itens mais vendidos (últimos 30 dias) — com fallback pra 1 por categoria
  // quando ainda não há histórico de vendas suficiente
  const highlights = useMemo(() => {
    const disponiveis = itens.filter((i) => i.disponivel && i.ativo)
    const porNome = new Map(disponiveis.map((i) => [i.nome, i]))
    const maisVendidos = topNomes
      .map((nome) => porNome.get(nome))
      .filter((i): i is ItemCardapio => !!i)

    if (maisVendidos.length > 0) return maisVendidos.slice(0, 5)

    return CATEGORIA_ORDER
      .flatMap((cat) => disponiveis.filter((i) => i.categoria === cat).slice(0, 1))
      .slice(0, 5)
  }, [itens, topNomes])
  ...
  function handleAdd(item: ItemCardapio, _e: React.MouseEvent) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F6F6F8' }}>
      ...
      <div style={{
        ...
        {/* Manchas de luz — dão profundidade sem repetir texto */}
        <div style={{
          position: 'absolute', width: 220, height: 220, borderRadius: 999,
          background: '#F4581C', opacity: 0.28, filter: 'blur(60px)',
          top: -90, left: -50,
        }} />
        <div style={{
          position: 'absolute', width: 180, height: 180, borderRadius: 999,
          background: '#3B5BA9', opacity: 0.22, filter: 'blur(60px)',
          top: -60, right: -40,
        }} />

        {/* Padrão sutil de ícones do cardápio */}
        <div style={{
        ...
        <div style={{
          ...
          background: 'linear-gradient(180deg, rgba(10,14,22,0.15) 0%, rgba(10,14,22,0) 40%, rgba(10,14,22,0.45) 100%)',
        }} />
      ...
      <div style={{
        ...
        <h1 style={{
          margin: 0, fontFamily: '"Pacifico", cursive', fontWeight: 400,
          fontSize: 32, lineHeight: 1, color: '#16202E', textAlign: 'center',
        }}>
          Luizao Lanches
        </h1>
        <p style={{ margin: '6px 0 0', color: '#6B7280', fontSize: 13, fontWeight: 500, textAlign: 'center' }}>
